#!/usr/bin/env python3
"""
YOLO inference program

This is a standard YOLO inference program that runs continuous inference loops.
XSched GPU scheduling is transparently applied via environment variables
configured by the launcher script, so this program doesn't need to know
about XSched at all.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='YOLO inference program with continuous inference loop',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run infinite loop with default model (yolov8n.pt, auto-downloaded)
  python yolo_xsched.py image.jpg
  
  # Run with custom model (will auto-download if not found)
  python yolo_xsched.py yolov8s.pt image.jpg
  
  # Run 100 iterations
  python yolo_xsched.py image.jpg --iterations 100
  
  # Run with 0.5 second interval between iterations
  python yolo_xsched.py image.jpg --interval 0.5
  
  # Process all images in a directory
  python yolo_xsched.py /path/to/images/ --iterations 50
  
  # Use CPU instead of GPU
  python yolo_xsched.py image.jpg --device cpu
        """
    )
    ...
    # YOLO will automatically download the model if it doesn't exist locally
    # No need to check if model file exists
